...
const buildAutoSku = (existingSkus: string[], category: string, name: string, color?: string) => {
  ...
  const nameCode = tokenPart(name, 'FLR');
  const colorCode = tokenPart(color || 'STD', 'STD');
  const prefix = `FL-${categoryCode}-${nameCode}-${colorCode}`;
...
const FloralPage = () => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['inventory_floral_items'] });
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-4xl h-[90vh] rounded-[28px] p-0 overflow-hidden flex flex-col">
          <DialogHeader className="px-6 pt-6 pb-4 border-b border-border/20 bg-white">
            <DialogTitle className="font-display text-2xl">{editing ? 'Editar item floral' : 'Novo item floral'}</DialogTitle>
          </DialogHeader>
          <div className="flex-1 overflow-y-auto px-6 py-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-muted-foreground">Padrão: <b>FL-CAT-NOM-COR-001</b> (automático).</p>
              </div>
              ...
              <div className="md:col-span-2 rounded-xl border border-gold/20 bg-gold/[0.04] p-4 space-y-3">
                <p className="text-[11px] font-black uppercase tracking-[0.16em] text-gold">Upload de fotos do floral</p>
