              ...
              <>
                <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Button variant="outline" onClick={() => setAddOpen(true)}><Plus size={15} className="mr-2" />Adicionar item</Button>
                  ...
                  <tr key={item.id} className="border-t border-border/30 text-sm">
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div><p className="font-semibold">{item.inventory_items?.name}</p><p className="text-xs text-muted-foreground">{item.inventory_items?.type === 'food' ? 'Alimentação' : 'Mobiliário'}</p></div>
                      </div>
                    </td>
                    <td className="p-4">{categoryLabel(item.inventory_items?.category || '')}</td>
                    <td className="p-4 font-semibold">{Number(item.quantity)}</td>
                    <td className="p-4">{Number(item.inventory_items?.available_quantity || 0)}</td>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="max-w-2xl rounded-[28px]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <Select value={selectedType} onValueChange={(v: any) => setSelectedType(v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Todas categorias</SelectItem><SelectItem value="food">Alimentação</SelectItem><SelectItem value="furniture">Mobiliário</SelectItem></SelectContent></Select>
          ...
          <div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setAddOpen(false)}>Cancelar</Button><Button onClick={() => addItemMutation.mutate()} disabled={!itemForm.itemId || !selectedReservation}>Reservar item</Button></div>
        </DialogContent>
      </Dialog>
