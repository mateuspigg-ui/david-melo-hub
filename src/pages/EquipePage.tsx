...
const EquipePage = () => {
  ...
  const createInvite = useMutation({
    mutationFn: async () => {
      ...
      if (inviteEmail) {
        await supabase.from('team_invitations').delete().eq('email', inviteEmail).eq('status', 'pending');
      }
    ...
    onSuccess: async (data: any) => {
      try {
        ...
        if (data?.email) {
          try {
            const { error: invokeError } = await supabase.functions.invoke('send-team-invite-email', {
              body: { email: data.email, inviteLink: link, modules: data.modules || [] },
            });
            if (invokeError) emailError = invokeError.message || 'Erro no serviço de e-mail';
          } catch (err: any) {
        ...
        toast({ title: 'Erro ao processar convite', description: err?.message || 'Ocorreu um erro inesperado.', variant: 'destructive' });
      }
    },
  ...
  const updatePermissions = useMutation({
    mutationFn: async ({ userId, modules, makeAdmin }: { userId: string; modules: string[]; makeAdmin: boolean }) => {
      ...
      if (makeAdmin) {
        ...
        const { error: removeMemberError } = await supabase.from('user_roles').delete().eq('user_id', userId).eq('role', 'team_member');
        if (removeMemberError) throw removeMemberError;
        return;
      }
      const { error: removeAdminError } = await supabase.from('user_roles').delete().eq('user_id', userId).eq('role', 'admin');
      ...
      const { error: modulesError } = await supabase.from('module_permissions').insert(safeModules.map(m => ({ user_id: userId, module: m })) as any);
    ...
    onError: (err: any) => {
      toast({ title: 'Erro ao atualizar permissões', description: err?.message || 'Não foi possível salvar as alterações.', variant: 'destructive' });
    },
  });

  const deleteInvite = useMutation({
    mutationFn: async (id: string) => { await supabase.from('team_invitations').delete().eq('id', id); },
  ...
  const removeMember = useMutation({
    mutationFn: async ({ userId, email }: { userId: string; email?: string | null }) => {
      ...
      if (email) {
        operations.push(Promise.resolve(supabase.from('team_invitations').delete().eq('email', email).eq('status', 'pending')));
      }
    ...
    onError: (err: any) => {
      toast({ title: 'Erro ao remover membro', description: err?.message || 'Não foi possível remover o membro da equipe.', variant: 'destructive' });
    },
  });
  ...
  if (!isAdmin) {
    return (
      <div className="page-container">
        <h1 className="page-header-title">Equipe</h1>
    ...
    <div className="page-container">
      <div className="page-header">
        <div>
          ...
          <span className="page-header-subtitle">David Melo Produções • Membros e Permissões</span>
        </div>
        <Button onClick={() => setShowInviteDialog(true)} className="page-action-button">
              ...
              <div key={member.id} className="bg-card rounded-xl p-5 border border-border/50 flex items-center justify-between hover:shadow-md transition-shadow">
                ...
                <div className="flex items-center gap-3">
                  <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full ${
                    member.roles?.includes('admin') ? 'bg-gold/10 text-gold' : 'bg-secondary text-muted-foreground'
                  }`}>
                  ...
                  {member.id !== user?.id && (
                    <>
                      <Button variant="ghost" size="icon" onClick={() => { setShowPermissionsDialog(member.id); setEditModules([]); setEditModulesDirty(false); setEditIsAdmin(member.roles?.includes('admin') || false); }} className="text-muted-foreground hover:text-gold" title="Permissões">
                        <Settings size={18} />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => { const confirmed = window.confirm(`Excluir o membro ${member.full_name || member.email || 'selecionado'} da equipe?`); if (!confirmed) return; removeMember.mutate({ userId: member.id, email: member.email }); }} className="text-muted-foreground hover:text-destructive" title="Excluir membro" disabled={removeMember.isPending}>
                        <Trash2 size={18} />
                      </Button>
                    </>
                  )}
              ...
              <div key={inv.id} className="bg-card rounded-xl p-4 border border-border/50 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider">{(inv.modules || []).length} módulos</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <Button variant="ghost" size="sm" onClick={() => { navigator.clipboard.writeText(getInviteLink(inv.token)); toast({ title: 'Link copiado!' }); }}>
      ...
      {/* Invite Dialog */}
      <Dialog open={showInviteDialog} onOpenChange={(open) => { setShowInviteDialog(open); if (!open) setGeneratedInviteLink(''); }}>
        <DialogContent className="max-w-md max-h-[90vh] flex flex-col">
          ...
          <div className="space-y-5 overflow-y-auto min-h-0">
            <div className="space-y-2">
              ...
              <Input value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} placeholder="colaborador@email.com" type="email" />
              ...
              <div className="grid gap-2 max-h-60 overflow-y-auto">
                {ALL_MODULES.map(mod => (
                  <label key={mod.key} className="flex items-center gap-3 p-2 rounded-lg hover:bg-secondary/30 cursor-pointer">
                    <Checkbox checked={selectedModules.includes(mod.key)} onCheckedChange={() => toggleModule(selectedModules, setSelectedModules, mod.key)} />
            ...
            {generatedInviteLink && (
              <div className="space-y-2 rounded-lg border border-gold/30 bg-gold/5 p-3">
                ...
                <div className="flex gap-2">
                  ...
                  <Button type="button" variant="outline" onClick={async () => { try { await navigator.clipboard.writeText(generatedInviteLink); toast({ title: 'Link copiado!' }); } catch { toast({ title: 'Não foi possível copiar automaticamente', description: 'Copie o link manualmente.', variant: 'destructive' }); } }}>
      ...
      <Dialog open={!!showPermissionsDialog} onOpenChange={(open) => { if (!open) { setShowPermissionsDialog(null); setEditModules([]); setEditModulesDirty(false); setEditIsAdmin(false); } }}>
        <DialogContent className="max-w-md max-h-[90vh] flex flex-col">
          <DialogHeader>
          ...
          <div className="rounded-lg border border-border/60 p-3 bg-secondary/20">
            <label className="flex items-center gap-3 cursor-pointer">
              <Checkbox checked={editIsAdmin} onCheckedChange={(value) => setEditIsAdmin(!!value)} />
            ...
            {ALL_MODULES.map(mod => (
              <label key={mod.key} className="flex items-center gap-3 p-2 rounded-lg hover:bg-secondary/30 cursor-pointer">
                <Checkbox
                  ...
                  onCheckedChange={() => { const current = editModulesDirty ? editModules : [...userPermissions]; setEditModulesDirty(true); toggleModule(current, setEditModules, mod.key); }}
                />
                <span className="text-sm">{mod.label}</span>
              </label>
            ))}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setShowPermissionsDialog(null); setEditModules([]); setEditModulesDirty(false); setEditIsAdmin(false); }}>Cancelar</Button>
            <Button onClick={() => updatePermissions.mutate({ userId: showPermissionsDialog!, modules: editModulesDirty ? editModules : userPermissions, makeAdmin: editIsAdmin })} disabled={updatePermissions.isPending} className="bg-gradient-gold text-white">
              Salvar Permissões
            </Button>
          </DialogFooter>
        </DialogContent>
